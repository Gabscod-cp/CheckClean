...
// Soft delete: mantém apartamentos e histórico intactos; eles ficam "sem proprietário".
export async function excluirProprietario(id) {
  const { error } = await supabase.from("proprietarios").update({ ativo: false }).eq("id", id);
  if (error) throw error;
}
...
export async function limpezasDoProprietario(token) {
  const { data, error } = await supabase.rpc("limpezas_do_proprietario", { p_token: token });
  if (error) throw error;
  return data;
}
